...
STORE.pagination = {
    ...
    Constructor: function(data) {
        ...
        STORE.pagination.activePage = data.page || 1;
    ...
    Click: function(evt) {
        ...
        STORE.pagination.activePage = +this.innerHTML;
    ...
    // previous page
    Prev: function() {
        STORE.pagination.activePage--;
        if ( STORE.pagination.activePage < 1) {
            STORE.pagination.activePage = 1;
        }
        STORE.pagination.functionClick();
        STORE.pagination.Start();
    ...
    // end
    End: function(){
        STORE.pagination.activePage = STORE.pagination.size;
        STORE.pagination.functionClick();
        STORE.pagination.Start();
    },
    ...
    Next: function() {
        STORE.pagination.activePage++;
        if ( STORE.pagination.activePage > STORE.pagination.size) {
            STORE.pagination.activePage = STORE.pagination.size;
        }
        STORE.pagination.functionClick();
        STORE.pagination.Start();
    ...
    Bind: function() {
        ...
        for (var i = 0; i < a.length; i++) {
            if (+a[i].innerHTML === STORE.pagination.activePage) a[i].className = 'current';
    ...
    Start: function() {
        ...
        else if (STORE.pagination.activePage < STORE.pagination.step * 2 + 1) {
            STORE.pagination.Add(1, STORE.pagination.step * 2 + 4);
            STORE.pagination.Last();
        }
        else if (STORE.pagination.activePage > STORE.pagination.size - STORE.pagination.step * 2) {
            STORE.pagination.First();
            STORE.pagination.Add(STORE.pagination.size - STORE.pagination.step * 2 - 2, STORE.pagination.size + 1);
        }
        else {
            STORE.pagination.First();
            STORE.pagination.Add(STORE.pagination.activePage - STORE.pagination.step, STORE.pagination.activePage + STORE.pagination.step + 1);
            STORE.pagination.Last();
        }
    ...
    getActivePage: function(){
        return STORE.pagination.activePage;
    }
};


STORE.removeChildren = function(padre){
    var node = $(padre);
