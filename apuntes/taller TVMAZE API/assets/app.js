  ...
  if (e.target.matches("#search")) {
    ...
    if (e.key === "Enter") {
      try {
        //alert("pepe");
        $shows.innerHTML =
          "'<img class='loader' src='./assets/SVG_loader/circles.svg' alt='Cargando...' />";
        ...
        if (json.length === 0) {
          ...
          json.forEach((el) => {
            ...
            $template.querySelector("img").style.maxWich = "100%";
        ...
        $shows.innerHTML = `<p> Error ${err.satus}: ${message}</p>`;
      }
    }
  }
});
